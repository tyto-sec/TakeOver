...
def get_hosts_with_permissive_spf(dns_file, domain_output_dir):
    if not os.path.isfile(dns_file):
        logging.error(f"DNS records file {dns_file} not found for SPF filtering.")
        return
    logging.info(f"Filtering SPF permissive candidates from DNS records in {dns_file}.")
    ...
    try:
        ...
        logging.info(f"Permissive SPF hosts saved to {spf_hosts_file}.")
    except Exception as e:
        logging.error(f"Error saving permissive SPF hosts: {e}")
...
def search_for_permissive_spf_hosts(dns_file):
    ...
    try:
        ...
        logging.error(f"Error searching DNS file {dns_file} for SPF hosts: {e}")
